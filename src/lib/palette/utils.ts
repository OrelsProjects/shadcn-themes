...
export function generatePalette({
  ...
  if (theme === "light") {
    ...
    baseForeground = text || [background[0], background[1], clampLightness(background[2] - 70)];
    ...
    baseForeground = text || [baseLightBackground[0], baseLightBackground[1], clampLightness(baseLightBackground[2] - 70)];
  ...
  const initialPrimaryForeground = primaryLightness < 50 
    ? adjustLightness(basePrimary, 80) 
    : adjustLightness(basePrimary, -80);
  const finalPrimaryForeground = ensureContrast(basePrimary, initialPrimaryForeground);

  const lightCard = baseCard;
  const lightCardForeground = ensureContrast(lightCard, adjustLightness(baseLightBackground, -10));
  ...
  const light: ThemePalette = {
    ...
    popover: ensureContrast(baseLightBackground, adjustLightness(baseLightBackground, 30)),
    "popover-foreground": ensureContrast(adjustLightness(baseLightBackground, 30), adjustLightness(baseLightBackground, -40)),

    primary: basePrimary,
    "primary-foreground": finalPrimaryForeground,

    secondary: baseSecondary,
    "secondary-foreground": ensureContrast(baseSecondary, adjustLightness(baseSecondary, -60)),

    muted: lightMuted,
    "muted-foreground": lightMutedForeground,

    accent: baseAccent,
    "accent-foreground": ensureContrast(baseAccent, adjustLightness(baseAccent, -60)),

    destructive: baseError,
    "destructive-foreground": ensureContrast(baseError, adjustLightness(baseError, +60)),
  ...
  // ===== Dark Mode Palette =====
  const darkForeground = ensureContrast(baseDarkBackground, adjustLightness(text || baseDarkBackground, +70));
  ...
  const darkPrimary = (theme === "light") ? adjustLightness(basePrimary, -30) : basePrimary;

  const darkPrimaryLightness = darkPrimary[2];
  const initialDarkPrimaryForeground = darkPrimaryLightness < 50
    ? adjustLightness(darkPrimary, 80)
    : adjustLightness(darkPrimary, -80);
  const finalDarkPrimaryForeground = ensureContrast(darkPrimary, initialDarkPrimaryForeground);

  // For secondary, accent, error in dark mode, we do the same logic:
  const darkSecondary = (theme === "light") ? adjustLightness(baseSecondary, -30) : baseSecondary;
  const darkAccent = (theme === "light") ? adjustLightness(baseAccent, -20) : baseAccent;
  const darkError = (theme === "light") ? adjustLightness(baseError, -20) : baseError;
  ...
  const dark: ThemePalette = {
    ...
    popover: ensureContrast(baseDarkBackground, adjustLightness(baseDarkBackground, -80)),
    "popover-foreground": ensureContrast(adjustLightness(baseDarkBackground, -80), adjustLightness(baseDarkBackground, +60)),

    primary: darkPrimary,
    "primary-foreground": finalDarkPrimaryForeground,

    secondary: ensureContrast(baseDarkBackground, darkSecondary),
    "secondary-foreground": ensureContrast(darkSecondary, adjustLightness(darkSecondary, +60)),

    muted: darkMuted,
    "muted-foreground": darkMutedForeground,

    accent: ensureContrast(baseDarkBackground, darkAccent),
    "accent-foreground": ensureContrast(darkAccent, adjustLightness(darkAccent, +60)),

    destructive: ensureContrast(baseDarkBackground, darkError),
    "destructive-foreground": ensureContrast(darkError, adjustLightness(darkError, +60)),

    border: ensureContrast(baseDarkBackground, adjustLightness(baseDarkBackground, -20)),
    input: ensureContrast(baseDarkBackground, adjustLightness(baseDarkBackground, +20)),
